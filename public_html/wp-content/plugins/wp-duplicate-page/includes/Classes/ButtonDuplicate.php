...
class ButtonDuplicate {
	...
	public function duplicateBulkLink( $bulkActions ) {
		if ( Utils::isCurrentUserAllowedToCopy() ) {
			...
			$bulkActions['wp_duplicate_page_bulk_action'] = esc_html( sprintf( __( '%s', 'njt_duplicate' ), $duplicateTextLink ) );
		}
		return $bulkActions;
	}
			...
			if ( is_array( $postIds ) ) {
				foreach ( $postIds as $postId ) {
					...
					if ( isset( $post ) && null !== $post ) {
						...
						wp_die( esc_html__( 'Copy creation failed, could not find original:', 'njt_duplicate' ) . ' ' . htmlspecialchars( $postId ) );
					}
				}
			}
	...
	public function duplicateButtonLink( $actions, $post ) {
		if ( Utils::isCurrentUserAllowedToCopy() && Utils::checkPostTypeDuplicate( $post->post_type ) ) {
			...
			$actions['njt_duplicate_page'] = sprintf(
				...
				esc_attr( __( 'Duplicate', 'njt_duplicate' ) ),
				/* translators: %s: Button Duplicate text. */
				esc_html( sprintf( __( ' %s ', 'njt_duplicate' ), $duplicateTextLink ) )
			);
			return $actions;
		}
		return $actions;
