...
namespace WPDataAccess\API {
	...
	class WPDA_Settings {
		...
		public static function save_admin_settings( $schema_name, $table_name, $settings ) {
			if (
				!
					(
						isset(
			...
			$admin_settings = self::get_admin_sttings_key(
				$settings['target'],
				$schema_name ,
				$table_name
			);
			...
			return WPDA_API::WPDA_Rest_Response(
				sprintf(
					__( 'Saved admin settings for database `%s` table `%s`', 'wp-data-access' ),
					$schema_name,
					$table_name
				)
			);
		...
		private static function get_admin_sttings_key( $target, $schema_name, $table_name ) {
			return self::WPDA_SETTINGS .
				'-' . $target .
